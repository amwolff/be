...
W kolejnych punktach zawarto najważniejsze definicje, które będą używane w
przeciągu całej pracy.
...
Jedną ze stosowanych\footnote{Metrykę tę stosowało na przykład badanie ,,How
	Unique Is Your Web Browser?'' Electronic Frontier Foundation, jedno z
największych badań tego typu.} metod pomiaru unikalności fingerprintu urządzeń i
przeglądarek jest entropia Shannona.
...
\(p(l_{i})\) to wyrażona ułamkiem częstość \(x_{i} \in D\) mającego etykietę
\(l_{i}\). W specjalnym przypadku w którym każda etykieta występuje tak samo
często entropia ma wartość maksymalną równą \(\log_{2}{(n - 1)}\).
