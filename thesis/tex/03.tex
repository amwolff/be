...
\emph{Fingerprinting} elementu \texttt{canvas} (HTML5) to technika
\emph{fingerprintingu}, która polega na prawidłowości tego, że przeglądarki
internetowe to aktualnie jedne z najbardziej skomplikowanych programów
komputerowych. Ściślejsze powiązanie przeglądarki z funkcjonalnością systemu
operacyjnego i podzespołami komputerowymi oznacza, że strony internetowe mają
większy dostęp do tych zasobów, a zachowanie przeglądarki internetowej różni się
w zależności od zachowania tych zasobów \cite[s. 1]{mowery2012pixel}. Pewnym
naturalnym schematem, aby zaimplementować niektóre zaawansowane funkcjonalności
przeglądarek, jest korzystanie z zasobów systemowych i sprzętowych hosta. Na
przykład rysowanie w elemencie \texttt{canvas} używając karty graficznej, daje
ogromny skok wydajnościowy i choćby większą oszczędność baterii w przypadku
urządzeń mobilnych.
...
\subsubsection{Algorytm (implementacja w JavaScript)}
